...
------
NGINX 란?
 Nginx(엔진엑스)는 igor Sysoev라는 러시아 개발자가 동시접속 처리에 특화된 웹 서버 프로그램이다. Apache보다 동작이 단순하고, 전달자 역할만 하기 때문에 동시접속 처리에 특화되어 있다. 즉 경량 웹 서버이다. HTTP Web Sever로 활용되기도 하고, Reverse Proxy Sever 로 활용하여 WAS서버의 부하를 줄일 수 있는 로드 밸러서로 활용되기도 한다.  
...
2. Nginx의 구조 
 Nginx는 하나의 Master process와 다수의 Worker Process로 구성되어 실행이 된다. Master Process는 설정 파일을 읽고, 유효성을 검사하고 Worker Process를 관리한다. 모든 요청은 Worker Process에서 처리한다. Nginx는 이벤트 기반 모델을 사용하고, Worker Process의 개수는 설정 파일에서 정의되며, 정의된 프로세스 개수와 사용 가능한 CPU 코어 숫자에 맞게 자동으로 조정이 된다. 
